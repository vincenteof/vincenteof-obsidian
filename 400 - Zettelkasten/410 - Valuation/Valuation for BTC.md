Date: 2025-11-12
Tags: [[remote/000 - Indexes/valuation]] [[macro]]

# Valuation for BTC

## Power
- [x] analyze influence of enery price increasement on bitcoin and mining company #active ✅ 2025-12-04

比特币货币化了能源，所有搁浅的电力最终都将转化为比特币。当挖矿成本小于比特币市场价的时候，这个过程会不断进行。当市场价明显低于电力成本时，从二级市场上直接买币更为划算，因此对比特币的价格产生了支撑。比特币本身的特性非常适合做储值工具，而作为储值工具它的价格又一定程度上增强它的价值，这里产生了自强化的循环，让上述货币化的过程可以不断重复，从而让比特币的价格可以不断锚定能源的价格。
数据中心的建立导致电力的巨大缺口，因此带来了新一轮的电力基建，因为比特币的存在你永远不用担心电力的过剩。每一闲置的 GW 都会流向 AI 训练或者 BTC。这里可能又存在另一个自强化的循环，这个正向过程中，矿场得到现金流后会扩张，从而再次升级电网更好地给数据中心供电。

Energy → Bitcoin → Compute 
Energy builds Bitcoin. 
Bitcoin monetizes energy. 
Energy + Bitcoin funds the grid. 
The grid powers AI. 
AI increases demand for more energy. 
Which Bitcoin again monetizes.

Hash Rate 是衡量整个比特币网络计算能力的指标，单位是 EH/s（每秒万亿亿次哈希计算）或者 ZH/s（1 ZH/s = 1000 EH/s）。哈希率越高，网络越安全，因为攻击它需要更多资源。哈希率代表了矿工投入的“真实努力”——包括电力、设备和基础设施。
在 2025/12/20，现状是 hash rate 很高，但是 BTC 价格却低迷。可能是因为当前价格被金融化工具影响，衍生品，杠杆，ETF等延缓了价值发现。但哈希率却持续稳步攀升。在某个时刻，价格必须与支撑网络的底层物理现实相协调。哈希是数字世界与物理世界连接的方式，它将能量转化为安全性和真实性。
所以这可能是一种 BTC 的未来的看涨信号？


## MSTR

- [x] analyze mstr USD reserve #active ✅ 2025-12-02

MSTR 预计通过 ATM 增发股票筹钱建立 USD 储备来支付股息和利息。目前这 14.4 亿美金至少足够支付 12 个月，因为在短期内不会出售 BTC。

- [x] 关于 MSTR 成为 BTC 银行 #active ✅ 2026-01-04

Saylor 自己发了一张摩根大通风格的肖像并配文：
**Bitcoin is money. Everything else is credit.**

很明显 Saylor 的野心是想做比特币储备的 **影子** 银行，当 BTC 的市值变的足够大时，他可能能成为一个真正的银行。目前它更像是 “金匠银行 / 商人银行的早期阶段”，它手里持有硬资产储备，对外发行凭证型负债。不同点是是历史上的“凭证”会进入支付/清算体系，逐渐货币化；而 MSTR 的负债目前主要是资本市场证券，**不直接承担支付与存款职能**。
一些决定天花板的硬约束：
1. 银行的核心是“稳定负债 + 可持续利差”，而 BTC 不生息。看 Saylor 是否能找到基于 BTC 产生稳定现金流的方法。
2. 监管与税务会把“影子银行”往阳光下拽
3. 最后贷款人（Lender of Last Resort）缺位，传统的银行可能有央行来处理流动性风险，而 MSTR 目前没有。

- [x] 分析 MSTR 的现金储备及永续债务和可能会发生的情况 #active ✅ 2026-06-08
截止 2026/05/27，USD 储备 8.7 亿，可转债总规模 67 亿（2028年后），永续债务每年的利息 8 亿。
目前获得 USD 储备的方式主要依赖资本市场“滚动融资”（持续发新 MSTR 普通股 + 新 STRC 等）。
如果卖币的话可以持续很久，只需要 STRC 回到 100 以上就能继续 ATM STRC 去维持 BTC 的价格。
MSTR 的最大的问题是完全暴露在了对手方的视野下，因为其他人不会让他很容易就过的舒服。相对而言它还有点缓冲垫，不会直接被价格快速下跌杀死，但是陷入漫长熊市它就会很被动。

## post quntum
量子计算可以做到从公钥计算出私钥，因为所有用公钥签名过的地址都有被破解风险，比如转账或参与只能合约。中本聪的公钥也是暴露的，一种可行的升级方案是提供窗口期进行升级，超过升级窗口期没升级的进行冻结。


# References
## energy
https://x.com/RakukageMSC/status/1990405761454010800
https://x.com/lanhubiji/status/1995695096654954966
## mstr
https://x.com/mindaoyang/status/1995531812345438591
https://x.com/cmdefi/status/1995630835614318854
https://x.com/hillery_dan/status/1999614537058689393
## post quantum
https://x.com/cr0ath/status/1971087482113655061?s=46&t=dB0iYuht_woxawlW9qX43A
## history
https://x.com/i/grok/share/d4Xu1Vg3Q2qWOQelPwsp9o5Rs
https://x.com/viviennaBTC/status/1997913951435444508
https://x.com/viviennaBTC/status/1997913951435444508

