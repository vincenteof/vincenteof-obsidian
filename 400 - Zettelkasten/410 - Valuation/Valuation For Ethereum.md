...
- [x] 总结 vitalik 最新关于 L2 的观点 #active ✅ 2026-02-09

L2 可能是以太坊生态的历史产物，大多数 L2 只能达到 Stage1 阶段（中等可信程度，权威实体依然有能力控制网络），甚至坦言它们不会进入 Stage2 阶段（和以太坊本身一样 trustless，没有中心化实体可以控制）。而且以太坊本身已经升级得足够便宜，可以容纳目前程度的网络活动。L2 如果只是简单扩容已经没有意义，它们可以去成为应用链或者带有其他特殊能力的网络（ZK，极端高可用）。L2 要么与 L1 保持一致（Native Rollup），因为 ZK 本身也对其了 L1 的发展方向，以太坊的开发者可用在 L1 中集成 precompile 的 ZK 基础函数，这样 L2 也更容易低成本地继承 L1 的 trustless，往 Stage2 发展。

本质上来说，从 2020 年确立 Rollup-centric 路线，产生了一种正和博弈：头部 L2 在这个过程中建立了独立的商业模式——排序器收入、代币经济和生态基金都跑通了；而 L1 则成功将扩容的工程成本和试错风险外部化，腾出精力专注协议层演进。
...
以太坊想实现的核心价值只有一点：实现真正的中立性，包括隐私、安全、开源、去中心化等等。AI越发达，对去中心化的需求越强烈，对中立性的需求规模越大，而这种需求，最终会传导到ETH的内在价值上。
https://x.com/lanhubiji/status/2033360299756310586
